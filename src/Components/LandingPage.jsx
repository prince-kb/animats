...
function LandingPage() {
  ...
  return (
    <div data-scroll data-scroll-speed="-.3" data-scroll-section className=' w-full h-screen bg-zinc-800 pt-2 z-8'>
        ...
        <div id='stars2'></div>
        <div id='stars3'></div>
      <div className={`${type === 'mobile' ? 'mt-[30vh]' : 'mt-[20vw]'} px-[4vw]`}>
        {["we create","eye opening","presentations"].map((item,index)=>(
            ...
            <h1 className={` ${type === 'mobile' ? 'leading-[12vw]' : 'leading-[7vw]'} uppercase font-['cavolini'] ${type === 'mobile' ? 'text-[5.5vh]' : 'text-[8vw]'} font-semibold tracking-tighter `} key={index}>{item}</h1>
         </div>
        ))}
