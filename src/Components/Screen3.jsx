                ...
                <div className='translate-x-[25vw] overflow-hidden text-[10vh] font-semibold text-blue-500 font-["Cavolini"] absolute z-20 '>
                    <h1 className='tracking-tighter inline-block'>{
                        "STUDY".split("").map((item,index)=>(
                            <motion.span key={index} className='inline-block' initial={{y : "100%"}} animate={anims[0]} transition={{delay : .05*index,ease : [0.33, 1, 0.68, 1]}} >{item}</motion.span>
                        ))
                    }</h1>
                </div>
            </motion.div>
                ...
                <div className='-translate-x-[25vw] overflow-hidden text-[10vh] font-semibold text-blue-500 font-["Cavolini"] absolute z-20 '>
                    <h1  className=' inline-block'>{
                        "LOVE".split("").map((item,index)=>(
                            <motion.span key={index} className='inline-block' initial={{y : "100%"}} animate={anims[1]} transition={{delay : .05*index,ease : [0.33, 1, 0.68, 1]}}>{item}</motion.span>
                        ))
                    }</h1>
                </div>
            </motion.div>
